...
def convert_BIO_labels(predictions):
    """
    convert_BIO_labels Function for NCBI is form:
...
#                 print('Processed %d sentences' % cnt)
    # return MultiLabelBinarizer().fit_transform(result_labels)
    return result_labels

def compute_metrics(predictions):
    """
    Metric Function for NCBI is from:
    https://github.com/justinphan3110/SciFive/blob/main/finetune/ner/scifive_fine_tune_ner.ipynb 
    """
...
def format_single_prompted_example(sentence: str) -> str:
    """Format a single input sentence. """
...
def preprocess_function(examples, tokenizer, max_seq_length, is_instruction_finetuned):
    ...
    if is_instruction_finetuned:
        ...
        inputs = ["ncbi_ner: " + x for x in examples['inputs']]
...
def get_data(ncbi_train_path: str, ncbi_dev_path: str, ncbi_test_path: str):
    """Get the ncbi data. """

    dataset_dict = {}
    dataset_dict['train'] = Dataset.from_pandas(pd.read_csv(ncbi_train_path))
    dataset_dict['val'] = Dataset.from_pandas(pd.read_csv(ncbi_dev_path))
    dataset_dict['test'] = Dataset.from_pandas(pd.read_csv(ncbi_test_path))
    
    return DatasetDict(dataset_dict)

def get_tokenized_data(dataset_dict: DatasetDict, tokenizer, max_seq_length: int, is_instruction_finetuned: bool):
    """Tokenize stuff. """
    tokenized_dict = {}
...
def train_model(model,
    ...
    decoded_predictions = tokenizer.batch_decode(outputs.predictions, skip_special_tokens=True)
    ...
    with open(output_dir + 'ncbi-prediction.txt', 'w+') as f:
        f.write("\n".join(predictions))
    with open(output_dir + 'ncbi-label.txt', 'w+') as f:
        f.write("\n".join(labels))
    with open(output_dir + 'ncbi-scores.json', 'w+') as f:
        json.dump(outputs.metrics, f)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--ncbi-train-path', type=str, required=True)
    parser.add_argument('--ncbi-dev-path', type=str, required=True)
    parser.add_argument('--ncbi-test-path', type=str, required=True)
    ...
    dataset_dict = get_data(args.ncbi_train_path, args.ncbi_dev_path, args.ncbi_test_path)
