...
https://github.com/elehman16/do-we-still-need-clinical-lms/blob/main/src/finetuning/run_seq2seq_qa.py
"""
...
class ModelArguments:
    ...
    model_name_or_path: str = field(
        metadata={"help": "path_to pretrained model or model identifier from huggingface.co/models"}
    )
    ...
    cache_dir: Optional[str] = field(
        default=None,
        metadata={"help": "path_to directory to store the pretrained models downloaded from huggingface.co"},
    )
...
class DataTrainingArguments:
    ...
    is_instruction_finetuned: bool = field(
        default=False,
        metadata={
            "help":"Whether the model is fine-tuned with instruction or not (for FLAN-T5)."
        },
    )
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argument to the script and it's the path_to a json file,
    ...
    is_instruction_finetuned = data_args.is_instruction_finetuned
    ...
    def preprocess_squad_batch(
        ...
        def generate_input(_question, _context, is_instruction_finetuned):
            if is_instruction_finetuned:
                return "Context: {} Question: {} If no answer is found in the context, do not reply; otherwise, give an answer from the context: ".format(_context.lstrip(),  _question.lstrip())
            else:
                return " ".join(["question:", _question.lstrip(), "context:", _context.lstrip()])

        inputs = [generate_input(question, context, is_instruction_finetuned) for question, context in zip(questions, contexts)]
        targets = [answer["text"][0] if len(answer["text"]) > 0 else "" for answer in answers]
        return inputs, targets

    def preprocess_function(examples):
        inputs, targets = preprocess_squad_batch(examples, question_column, context_column, answer_column, is_instruction_finetuned)
    ...
    if training_args.do_predict:
        ...
        with open(training_args.output_dir + 'radqa-prediction.txt', 'w+') as f:
            for line in results.predictions:
                f.write(f"{line}\n")
        with open(training_args.output_dir + 'radqa-label.txt', 'w+') as f:
            for line in results.label_ids:
                f.write(f"{line}\n")
        with open(training_args.output_dir + 'radqa-scores.json', 'w+') as f:
